function initialize() {
  ...
  var customIcon = L.icon({
    iconUrl: "images/mountain-bike.png",
  ...
  $.getJSON(
    "https://bicycleinfo0803.herokuapp.com/coordinates",
    function (data) {
      for (var i = 0; i < data.coordinates.length; i++) {
        L.marker([data.coordinates[i].lat, data.coordinates[i].lon], {
          icon: customIcon,
        })
          .bindPopup(data.coordinates[i].key)
          .openPopup()
          .addTo(map);
      }
    }
